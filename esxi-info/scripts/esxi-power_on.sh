...
# Función para obtener el VMID y encender la máquina virtual
power_on_vm() {
    ...
    # Intenta encender la máquina virtual
    ssh "$USER@$HOST" "vim-cmd vmsvc/power.on $VM_ID" \
        && log_info "La máquina virtual \"$VM_NAME\" ha sido encendida." \
        || log_error "Error al intentar encender la máquina virtual \"$VM_NAME\"."
}

# Ejecuta la función para encender la máquina virtual
power_on_vm
